...
class Analyze:
    ...
    def analyze_xls(self, filename):
        ...
        #         if(sh.range('A'+str(x)).value is not None):
        #             if(item.getName()==sh.range('A'+str(x)).value):
        #                 item.addNewShipment(shipment=Shipment(sh.range('B'+str(x)).value),count=sh.range('C'+str(x)).value)
        ...
        for item in self.otdels:
            for x in range(2,self.rownum):
                if(item.getName().lower() ==self.dict_name_otdel[x]):
                    item.addNewShipment(shipment=Shipment(self.dict_imushestvo[x]),count=self.dict_kolvo[x])
